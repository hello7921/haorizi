...
class Page {
    ...
    public function hshow() {
        ...
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<a href='index".$upRow.".html'>".$this->config['prev']."</a>";
        }else{
            $upPage     =   '';
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<a href='index".$downRow.".html'>".$this->config['next']."</a>";
        }else{
            $downPage   =   '';
        }
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a href='index.html' >".$this->config['first']."</a>";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
                $theEndRow  =   $this->totalPages;
                $theEnd     =   "<a href='index".$theEndRow."html' >".$this->config['last']."</a>";
            }
        }
        ...
        if ($this->totalPages != 1) {
            ...
            for ($page = $start; $page <= $end; $page++) {
                if ($page != $this->nowPage) {
                    $linkPage .= " <a href='index".$page.".html'>&nbsp;".$page."&nbsp;</a>";
                } else {
                    $linkPage .= " <span class='current'>".$page."</span>";
                }
            }
